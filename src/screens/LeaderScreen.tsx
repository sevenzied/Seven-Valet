...
export function LeaderScreen({ user, onLogout }: Props) {
  ...
  const parking   = requests.filter(r => ["parking", "accepted"].includes(r.status)).length;
  const parked    = requests.filter(r => r.status === "parked").length;
  const displayed = tab === "live" ? active : requests;
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        <StatCard label="Total"    value={requests.length} color={Colors.textSub} icon="📋" />
      </ScrollView>

      {/* Tabs */}
      <View style={styles.tabs}>
        <Pill label={`🔴 Live (${active.length})`} active={tab === "live"} onPress={() => setTab("live")} />
        <Pill label={`All (${requests.length})`}    active={tab === "all"}  onPress={() => setTab("all")} />
      </View>
